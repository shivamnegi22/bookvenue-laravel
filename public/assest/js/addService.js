...
addButton.addEventListener("click", function (event) {
    ...
    if (serviceValue === "") {
        ...
        form.querySelectorAll(
            '[name="court_name"], .startTime, .endTime, [name="prize"]'
        ).forEach(function (field) {
            field.addEventListener("input", function () {
                courtData.name = form.querySelector(
                    '[name="court_name"]'
                ).value;
        ...
        form.querySelectorAll(
            '[name="court_name"], .startTime, .endTime'
        ).forEach(function (field) {
            field.addEventListener("input", function () {
                calculateDuration();
                validateDuration();
        ...
        form.querySelector(".add").addEventListener("click", function () {
          ...
          `;
            ...
            timeGroup
                .querySelectorAll(".startTime, .endTime")
                .forEach(function (field) {
                    field.addEventListener("input", function () {
                        breakTime.start_Time = timeGroup.querySelector(
                            '[name="start_Time"]'
                        ).value;
                        breakTime.end_Time =
                            timeGroup.querySelector('[name="end_Time"]').value;
                    });
                });
        ...
        form.querySelectorAll(".duration-control").forEach(function (button) {
            button.addEventListener("click", function () {
                ...
                var minutes = parseInt(
                    currentDuration.split(" ")[1].split("m")[0],
                    10
                );
        ...
        function validateDuration() {
            ...
            calculatedEndTime.setMinutes(
                calculatedEndTime.getMinutes() + minutes
            );
